...
def DualResNet_imagenet(pretrain_path, num_classes, pretrained=False):
...
class DDRNet23SlimImageNet(nn.Module):
    def __init__(self, pretrain_path, num_classes, pretrained=True):
        super(DDRNet23SlimImageNet, self).__init__()
        self.model = DualResNet_imagenet(pretrain_path=pretrain_path, num_classes=num_classes, pretrained=pretrained)
...
if __name__ == '__main__':
    ...
    net = DualResNet_imagenet(pretrain_path='./pretrained_models/DDRNet23s_imagenet.pth', pretrained=True)
    y = net(x)
    print(np.shape(y[1]))
