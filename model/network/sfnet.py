...
class SFNet_ResNet(nn.Module):

    def __init__(self, block, layers, num_classes=7, scale=1):
        self.inplanes = 64
        super(SFNet_ResNet, self).__init__()
        ...
        # flow layer
        self.flowconv1 = nn.Conv2d(512, 2, kernel_size=3, stride=1, padding=1)
        self.flowconv2 = nn.Conv2d(512, 2, kernel_size=3, stride=1, padding=1)
        self.flowconv3 = nn.Conv2d(512, 2, kernel_size=3, stride=1, padding=1)
    ...
    # Flow Align Module
    def _flow_align_module(self, featmap_front, featmap_latter, func):
    ...
    def forward(self, x):
        ...
        p5_flow = self._flow_align_module(c4, p5, self.flowconv1)
        p4 = self._upsample_add(p5_flow, c4)
        p4 = self.smooth1(p4)
        p4 = self.smooth1_relu(self.smooth1_bn(p4))

        c3 = self.latlayer2(c3)
        c3 = self.latlayer2_relu(self.latlayer2_bn(c3))
        p4_flow = self._flow_align_module(c3, p4, self.flowconv2)
        p3 = self._upsample_add(p4_flow, c3)
        p3 = self.smooth2(p3)
        p3 = self.smooth2_relu(self.smooth2_bn(p3))

        c2 = self.latlayer3(c2)
        c2 = self.latlayer3_relu(self.latlayer3_bn(c2))
        p3_flow = self._flow_align_module(c2, p3, self.flowconv3)
        p2 = self._upsample_add(p3_flow, c2)
        p2 = self.smooth3(p2)
...
def sf_resnet18(pretrained=False, num_classes=19, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = SFNet_ResNet(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['sf_resnet18']))
    return model


class SFResNet18(nn.Module):
    def __init__(self, num_classes=19):
        super(SFResNet18, self).__init__()
        self.model = sf_resnet18(num_classes=num_classes)

    def forward(self, x):
        return self.model(x)


def sf_resnet34(pretrained=False, **kwargs):
    """Constructs a ResNet-34 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = SFNet_ResNet(BasicBlock, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['sf_resnet34']))
    return model


def sf_resnet50(pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = SFNet_ResNet(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        pretrained_model = model_zoo.load_url(model_urls['sf_resnet50'])
...
def sf_resnet101(pretrained=False, **kwargs):
    """Constructs a ResNet-101 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = SFNet_ResNet(Bottleneck, [3, 4, 23, 3], **kwargs)
    if pretrained:
        pretrained_model = model_zoo.load_url(model_urls['sf_resnet101'])
        state = model.state_dict()
        for key in state.keys():
            if key in pretrained_model.keys():
                state[key] = pretrained_model[key]
...
def sf_resnet152(pretrained=False, **kwargs):
    """Constructs a ResNet-152 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = SFNet_ResNet(Bottleneck, [3, 8, 36, 3], **kwargs)
    if pretrained:
        pretrained_model = model_zoo.load_url(model_urls['sf_resnet152'])
        state = model.state_dict()
        for key in state.keys():
            if key in pretrained_model.keys():
                state[key] = pretrained_model[key]
